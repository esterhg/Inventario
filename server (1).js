...
// Los valores se leen de variables de entorno (para poder configurarlos en
// Coolify sin tocar el código). Si no existen, se usan los valores originales
// como respaldo para que siga funcionando en local.
const pool = new Pool({
  host:     process.env.DB_HOST || 'localhost',
  port:     Number(process.env.DB_PORT) || 5432,
  database: process.env.DB_NAME || 'control_activos_occ',
  user:     process.env.DB_USER || 'postgres',
  password: process.env.DB_PASSWORD || 'Insta2025',
});
// ───────────────────────────────────────────────────────────────────────────
